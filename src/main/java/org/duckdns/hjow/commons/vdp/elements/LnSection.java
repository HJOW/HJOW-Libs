...
public class LnSection extends Section 
{
	private static final long serialVersionUID = -304606408625166904L;
	
	public LnSection(Tracker tracker)
	{
		super(tracker);
	}
	@Override
	...
	public Section differential() 
	{
		...
		LnSection newLn = (LnSection) this.clone();
		newOne.previousOperation = this.previousOperation;
		newLn.previousOperation = '+';
		if(this.exponent.data == 1.0)
		{
			...
			else if(inside.getDataType() == Data.SECTION) two = (Section) ((SectionData) inside).contents.clone();
			else return Section.initialize(Section.ZERO);
			two.exponent.data *= -1.0;
		...
		else
		{
			...
			newLn.exponent.data = 1.0;
			two = newLn.differential();
			if(inside.getDataType() == Data.DOUBLE) return Section.initialize(Section.ZERO);
	...
	{
		LnSection newOne = (LnSection) Section.initialize(Section.LN);
	...
	{
		return Tracker.TRACKABLE_LN_SECTION;
	}
}

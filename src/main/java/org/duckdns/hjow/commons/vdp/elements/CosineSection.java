...
public class CosineSection extends Section
{
	private static final long serialVersionUID = 4967732618704423765L;

	public CosineSection(Tracker tracker)
	{
		super(tracker);
	}
	@Override
	...
	public Section differential() 
	{
		...
		else
		{
			...
			if(this.exponent.data == 1.0)
			{
				...
				firstDerivate.coefficient.data = this.coefficient.data * -1.0;
			...
			else
			{
				...
				SineSection secondDerivate = (SineSection) Section.initialize(Section.SINE);
	...
	{
		CosineSection newone = (CosineSection) Section.initialize(Section.COSINE);
	...
	public DoubleData calculate(DoubleData value)
	{
		...
		if(inside.getDataType() == Data.DOUBLE) newOne.data = coefficient.data * Math.pow(Math.cos(((DoubleData)inside).data), exponent.data);
		else if(inside.getDataType() == Data.SECTION)
		{
			newOne.data = coefficient.data * Math.pow(Math.cos(((SectionData) inside).contents.calculate(value).data), exponent.data);
		}
		return newOne;
	}
	@Override
	...
	{
		return Tracker.TRACKABLE_COSINE_SECTION;
	}

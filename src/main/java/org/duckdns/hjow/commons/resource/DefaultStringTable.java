...
/** 언어 지원을 위한 String Table, 모든 데이터가 메모리에 상주 */
public class DefaultStringTable implements StringTable {
	...
	public DefaultStringTable(File file) {
	...
	public DefaultStringTable(String resources, Class<?> sameResourcePathClass) {
	...
	/** 기본 스트링 테이블 등록 (이 클래스와 동일 경로 내에 있는 stringTable.xml) */
	public static StringTable register() { return registerResource("stringTable.xml", StringTable.class); }
	/** 스트링 테이블 등록 */
	...
	public static StringTable register(File file) {
		StringTable table = new DefaultStringTable(file);
	...
	public static StringTable registerResource(String resourceName, Class<?> sameResourcePathClass) {
		StringTable table = new DefaultStringTable(resourceName, sameResourcePathClass);
		stringTables.put(table.getName(), table);
		return table;
	}
